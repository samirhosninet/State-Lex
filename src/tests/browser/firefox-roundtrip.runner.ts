...
export async function runFirefoxVerification(): Promise<void> {
  const distDir = join(process.cwd(), 'dist');

  if (!existsSync(join(distDir, 'index.html'))) {
    throw new Error("dist/index.html missing. Run 'npm run build' before executing Firefox test.");
  }
  ...
  try {
    ...
    console.log(`Browser Used: Firefox`);
